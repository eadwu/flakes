...
old_rev="$(nix show-derivation "$old_src_drv" | jq -r '.[].env.urls' | grep -Po '(?<=archive/).+(?=.tar.gz)')"
repo="$(nix show-derivation "$old_src_drv" | jq -r '.[].env.urls' | sed 's@/archive.*@@')"
