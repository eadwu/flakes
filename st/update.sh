...
new_version="$(cd $tmpdir && git log -1 --format=%cs)"
new_revision="$(cd "$tmpdir" && git rev-parse HEAD)"
...
old_rev_escaped="$(echo "$old_rev" | sed -re 's|[.+]|\\&|g')"
...
sed -i "$source_file" -re "s|\"$old_rev_escaped\"|\"$new_rev\"|"
